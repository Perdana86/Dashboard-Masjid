...
const AdminPage = () => {
  ...
  const filteredCities = useMemo(() => {
    ...
    const list = q
      ? cities.filter((c) => c.lokasi.toLowerCase().includes(q))
      : cities;
  ...
  const saveBio = async (e) => {
    ...
    try {
      ...
      [
        ...
        "operating_hours",
        "long_description",
      ].forEach((k) => fd.append(k, bioForm[k] || ""));
  ...
  const deleteBioPhoto = async () => {
    ...
    try {
      const saved = await pb
        .collection("bio")
        .update(bioRecord.id, { photo: null });
      setBioRecord(saved);
      setBioForm({ ...saved });
      setBioPhotoFile(null);
  ...
  const saveSettings = async (e) => {
    ...
    try {
      ...
      fd.append(
        "sched_before_prayer",
        String(Number(form.sched_before_prayer) || 5),
      );
      fd.append(
        "sched_after_prayer",
        String(Number(form.sched_after_prayer) || 10),
      );
      fd.append(
        "sched_interval_hours",
        String(Number(form.sched_interval_hours) || 1),
      );
      fd.append(
        "sched_enabled",
        form.sched_enabled === false ? "false" : "true",
      );
      const numOrZero = (v) => {
        const n = Number(v);
  ...
  const deleteFavicon = async () => {
    ...
    try {
      const saved = await pb
        .collection("settings")
        .update(settings.id, { favicon: null });
      setSettings(saved);
      setForm({ ...saved });
      setFaviconFile(null);
      ...
      if (!isAbortError(err))
        setError(err.message || "Gagal menghapus favicon");
    } finally {
  ...
  const deletePwaLogo = async () => {
    ...
    try {
      const saved = await pb
        .collection("settings")
        .update(settings.id, { pwa_logo: null });
      setSettings(saved);
      setForm({ ...saved });
      setPwaLogoFile(null);
      ...
      if (!isAbortError(err))
        setError(err.message || "Gagal menghapus logo PWA");
    } finally {
  ...
  return (
    <div className="min-h-[100dvh] bg-[var(--m-bg)] text-emerald-50">
        ...
        {tabs.map((t) => (
          <button
            ...
            className={`-mb-px border-b-2 px-4 py-3 text-sm transition-colors ${
              tab === t.id
                ? "border-[var(--m-primary)] text-[var(--m-primary)]"
                : "border-transparent text-emerald-100/55 hover:text-white"
            }`}
          >
            {t.label}
          </button>
      ...
      <main className="px-6 py-8 md:px-10">
        ...
        {error ? (
          <p className="mb-5 border border-red-400/30 bg-red-400/10 px-4 py-2 text-sm text-red-200">
            {error}
          </p>
        ) : null}
        ...
        {loading || !form ? (
          ...
          <form
            ...
            className="flex max-w-6xl flex-col gap-5 pb-4"
          >
            {/* Identitas Masjid */}
            <section className="grid gap-4 lg:grid-cols-3">
              <h3 className="text-xs uppercase tracking-[0.25em] text-[var(--m-primary)] lg:col-span-3">
                Identitas Masjid
              </h3>
              <div className="flex flex-col gap-1.5">
                <label className={labelClass} htmlFor="mosque_name">
                  Nama Masjid
                </label>
                <input
                  id="mosque_name"
                  ...
                  onChange={(e) =>
                    setForm({ ...form, mosque_name: e.target.value })
                  }
                />
              </div>
              <div className="flex flex-col gap-1.5">
              ...
              <div className="flex flex-col gap-1.5">
                <label className={labelClass} htmlFor="slide_seconds">
                  Durasi Slide (detik)
                </label>
                <input
                  id="slide_seconds"
                  ...
                  max="120"
                  className={inputClass}
                  value={form.slide_seconds || 8}
                  onChange={(e) =>
                    setForm({ ...form, slide_seconds: e.target.value })
                  }
                />
              </div>
            ...
            <section className="grid gap-4 lg:grid-cols-2">
              <h3 className="text-xs uppercase tracking-[0.25em] text-[var(--m-primary)] lg:col-span-2">
                Media &amp; Ikon
              </h3>
              <div className="flex flex-col gap-1.5">
                ...
                {settings && settings.logo ? (
                  <img
                    ...
                    alt="Logo saat ini"
                    className="mt-1.5 h-12 w-12 rounded-full object-cover ring-1 ring-[color-mix(in_srgb,var(--m-primary)_50%,transparent)]"
                  />
                ) : null}
              </div>

              <div className="flex flex-col gap-1.5">
                <label className={labelClass} htmlFor="favicon">
                  ...
                  {faviconFile ? (
                    <div className="flex items-center gap-2">
                      <img
                        ...
                        className="h-9 w-9 rounded object-contain bg-white/5 ring-1 ring-white/10"
                      />
                      <span className="text-xs text-emerald-100/55">
                        {faviconFile.name} (belum disimpan)
                      </span>
                    </div>
                  ) : settings && settings.favicon ? (
                    <div className="flex items-center gap-2">
                      <img
                        src={fileUrl(settings, settings.favicon)}
                        alt="Favicon saat ini"
                        className="h-9 w-9 rounded object-contain bg-white/5 ring-1 ring-[color-mix(in_srgb,var(--m-primary)_50%,transparent)]"
                      />
                      <span className="text-xs text-emerald-100/55">
                        Favicon saat ini
                      </span>
                    ...
                    <span className="text-xs text-emerald-100/45">
                      Belum ada favicon. PNG/JPG/SVG/ICO, maks. 5 MB.
                    </span>
                  )}
            ...
            <section className="grid gap-4 lg:grid-cols-2">
              ...
              <div className="flex flex-col gap-1.5">
                <label className={labelClass} htmlFor="running_text">
                  Running Teks (pisahkan tiap pesan dengan |)
                </label>
                <textarea
                  id="running_text"
                  rows={2}
                  ...
                  onChange={(e) =>
                    setForm({ ...form, running_text: e.target.value })
                  }
                />
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className={labelClass} htmlFor="quote_source">
                    Sumber Kutipan
                  </label>
                  <input
                    id="quote_source"
                    ...
                    onChange={(e) =>
                      setForm({ ...form, quote_source: e.target.value })
                    }
                  />
                </div>
              </div>
            ...
            <section className="grid gap-4 lg:grid-cols-2">
              <h3 className="text-xs uppercase tracking-[0.25em] text-[var(--m-primary)] lg:col-span-2">
                Kota Jadwal Sholat (API myquran)
              </h3>
              <div className="flex flex-col gap-1.5">
                <label className={labelClass} htmlFor="city_search">
                  Cari Kota
                </label>
                <input
                  id="city_search"
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setForm({
                      ...
                      city_name: city ? city.lokasi : form.city_name,
                    });
                  }}
                >
                  <option value={form.city_id || ""}>
                    {form.city_name || "Pilih kota"}
                  </option>
                ...
                {cities.length === 0 ? (
                  <p className="text-xs text-emerald-100/45">
                    Daftar kota belum termuat, ID kota saat ini tetap dipakai.
                  </p>
                ) : null}
              </div>
            </section>
            ...
            <div className="sticky bottom-0 -mx-6 mt-1 border-t border-white/10 bg-[color-mix(in_srgb,var(--m-bg)_92%,transparent)] px-6 py-3 backdrop-blur md:-mx-10 md:px-10">
              <div className="flex flex-wrap items-center justify-end gap-3">
                <span className="mr-auto text-xs text-emerald-100/50">
                  Perubahan diterapkan real-time ke dashboard setelah disimpan.
                </span>
                <button
                  type="submit"
                  disabled={saving}
            ...
            <form onSubmit={saveBio} className="flex flex-col gap-6 pb-4">
              <div>
                ...
                <p className="mt-1 text-sm text-emerald-100/55">
                  Konten profil masjid yang tampil di halaman /bio. Perubahan
                  tersimpan ke database dan sinkron real-time ke halaman bio.
                </p>
              </div>
              ...
              <section className="grid gap-4 lg:grid-cols-2">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className={labelClass} htmlFor="bio_mosque_name">
                    Nama Masjid
                  </label>
                  <input
                    id="bio_mosque_name"
                    className={inputClass}
                    value={bioForm?.mosque_name || ""}
                    onChange={(e) =>
                ...
                <div className="flex flex-col gap-1.5">
                  <label className={labelClass} htmlFor="bio_email">
                    Email
                  </label>
                  <input
                    id="bio_email"
                ...
                <div className="flex flex-col gap-1.5 lg:col-span-2">
                  <label className={labelClass} htmlFor="bio_description">
                    Deskripsi Singkat
                  </label>
                  <textarea
                    id="bio_description"
                    ...
                    onChange={(e) =>
                      setBioForm({ ...bioForm, description: e.target.value })
                    }
                    placeholder="Ringkasan satu kalimat tentang masjid."
                  />
                </div>
              </section>
              ...
              <section className="grid gap-4 lg:grid-cols-2">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className={labelClass} htmlFor="bio_address">
                    Alamat Lengkap
                  </label>
                  <textarea
                    id="bio_address"
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="flex flex-col gap-1.5">
                    <label className={labelClass} htmlFor="bio_phone">
                      Nomor Telepon
                    </label>
                    <input
                      id="bio_phone"
                      ...
                      value={bioForm?.phone || ""}
                      onChange={(e) =>
                        setBioForm({ ...bioForm, phone: e.target.value })
                      }
                      placeholder="+62 21 1234 5678"
                    />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <label className={labelClass} htmlFor="bio_operating_hours">
                      Jam Operasional
                    </label>
                    <input
                      id="bio_operating_hours"
                      className={inputClass}
                      value={bioForm?.operating_hours || ""}
                      onChange={(e) =>
                        setBioForm({
                          ...bioForm,
                      ...
                      placeholder="Setiap hari, 04:00 - 21:00"
                    />
                  </div>
                </div>
              </section>
              ...
              <section className="grid gap-4">
                ...
                <div className="flex flex-col gap-3 border border-white/10 bg-white/[0.03] p-5">
                  <label className={labelClass} htmlFor="bio_photo">
                    Unggah Foto (PNG/JPG/WebP/SVG/GIF/AVIF, maks. 10 MB)
                  </label>
                  <input
                    id="bio_photo"
                    type="file"
                    accept="image/png,image/jpeg,image/webp,image/gif,image/svg+xml,image/avif"
                    onChange={(e) => setBioPhotoFile(e.target.files[0] || null)}
                    className={inputClass}
                  />
                  <div className="mt-1 flex flex-wrap items-center gap-4">
                    {bioPhotoFile ? (
                      <div className="flex items-center gap-3">
                        <img
                          src={URL.createObjectURL(bioPhotoFile)}
                          alt="Pratinjau foto"
                          className="h-24 w-40 rounded object-cover ring-1 ring-white/10"
                        />
                        <span className="text-xs text-emerald-100/55">
                          {bioPhotoFile.name} (belum disimpan)
                        </span>
                      </div>
                    ) : bioRecord && bioRecord.photo ? (
                      <div className="flex items-center gap-3">
                        <img
                          src={fileUrl(bioRecord, bioRecord.photo)}
                          alt="Foto saat ini"
                          className="h-24 w-40 rounded object-cover ring-1 ring-[color-mix(in_srgb,var(--m-primary)_50%,transparent)]"
                        />
                        <span className="text-xs text-emerald-100/55">
                          Foto saat ini
                        </span>
              ...
              <section className="grid gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className={labelClass} htmlFor="bio_long_description">
                    Teks Tambahan / Tentang Masjid
                  </label>
                  <textarea
                    id="bio_long_description"
                    rows={8}
                    className={inputClass}
                    value={bioForm?.long_description || ""}
                    onChange={(e) =>
                      setBioForm({
                        ...bioForm,
                    ...
                    placeholder="Sejarah, visi, program, dan informasi lengkap tentang masjid. Pisahkan paragraf dengan baris baru."
                  />
                  <p className="text-xs text-emerald-100/45">
                    Paragraf dipisahkan dengan baris baru (Enter).
                  </p>
                </div>
              </section>
              ...
              <div className="sticky bottom-0 -mx-6 mt-1 border-t border-white/10 bg-[color-mix(in_srgb,var(--m-bg)_92%,transparent)] px-6 py-3 backdrop-blur md:-mx-10 md:px-10">
                <div className="flex flex-wrap items-center justify-end gap-3">
                  <span className="mr-auto text-xs text-emerald-100/50">
                    Perubahan diterapkan real-time ke halaman /bio setelah
                    disimpan.
                  </span>
                  <button
                    type="submit"
          ...
          <div className="max-w-5xl space-y-6">
            <div>
              ...
              <p className="mt-1 text-sm text-emerald-100/55">
                ...
                negatif (contoh: -13.937.817). Data tersimpan ke database dan
                tampil real-time di dashboard.
              </p>
            </div>

            <div className="flex flex-col gap-2 max-w-xl">
              <label className={labelClass} htmlFor="saldo_label">
                Judul Informasi
              </label>
              <input
                id="saldo_label"
              ...
              {[
                ...
                >
                  <div className="flex flex-col gap-2">
                    <label className={labelClass} htmlFor={col.labelId}>
                      {col.labelTitle}
                    </label>
                    <input
                      id={col.labelId}
                      ...
                      onChange={(e) =>
                        setForm({ ...form, [col.labelKey]: e.target.value })
                      }
                  ...
                  <div className="flex flex-col gap-2 border-t border-white/10 pt-4">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm({ ...form, [col.valueKey]: e.target.value })
                      }
                    />
                    <p
                      className={`text-xs font-num ${col.preview < 0 ? "text-red-300/80" : "text-emerald-100/45"}`}
                    >
            ...
            <div className="flex flex-wrap items-center gap-4">
              <button
                ...
                onClick={() =>
                  setForm({
                    ...form,
                    saldo_visible: form.saldo_visible === false,
                  })
                }
              ...
              <span className="text-sm text-emerald-100/50">
                Status:{" "}
                {form.saldo_visible === false ? "Disembunyikan" : "Ditampilkan"}
              </span>
            </div>
          ...
          <div className="max-w-3xl space-y-6">
            <div>
              <h2 className="font-display text-xl text-white">
                Penjadwalan Otomatis Slide
              </h2>
              <p className="mt-1 text-sm text-emerald-100/55">
                Atur kapan Dashboard menampilkan Slide Dashboard atau Slide
                Layar Informasi.
              </p>
            </div>
            ...
            <label className="flex items-center gap-3 text-sm text-emerald-100/80">
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, sched_enabled: e.target.checked })
                }
              />
            ...
            <div className="grid gap-6 sm:grid-cols-3">
              <div className="flex flex-col gap-2">
                <label className={labelClass} htmlFor="sched_before_prayer">
                  Menit sebelum sholat (Slide Dashboard)
                </label>
                <input
                  id="sched_before_prayer"
                  type="number"
                  min="1"
                  max="60"
                  className={inputClass}
                  ...
                  onChange={(e) =>
                    setForm({ ...form, sched_before_prayer: e.target.value })
                  }
                />
              </div>
              <div className="flex flex-col gap-2">
                <label className={labelClass} htmlFor="sched_after_prayer">
                  Menit setelah sholat (Slide Informasi)
                </label>
                <input
                  id="sched_after_prayer"
                  type="number"
                  min="1"
                  max="60"
                  className={inputClass}
                  ...
                  onChange={(e) =>
                    setForm({ ...form, sched_after_prayer: e.target.value })
                  }
                />
              </div>
              <div className="flex flex-col gap-2">
                <label className={labelClass} htmlFor="sched_interval_hours">
                  Interval rotasi (jam)
                </label>
                <input
                  id="sched_interval_hours"
                  type="number"
                  min="1"
                  max="24"
                  className={inputClass}
                  ...
                  onChange={(e) =>
                    setForm({ ...form, sched_interval_hours: e.target.value })
                  }
                />
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="mt-1 text-sm text-emerald-100/55">
                Pilih warna latar, panel, aksen, dan teks. Perubahan diterapkan
                langsung di seluruh dashboard saat disimpan.
              </p>
            </div>
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(320px,440px)] xl:items-start">
              ...
              <div className="space-y-6">
                <div className="grid gap-4 sm:grid-cols-2">
                  {THEME_FIELDS.map((c) => (
                    <div
                      key={c.key}
                      className="flex flex-col gap-3 border border-white/10 bg-white/[0.03] p-5"
                    >
              ...
              <div className="border border-white/10 bg-white/[0.03] p-4 sm:p-5 xl:sticky xl:top-4">
                ...
                <div
                  ...
                  style={{
                    background: form.theme_bg || THEME_DEFAULTS.theme_bg,
                  }}
                >
                  {/* Mini header */}
                  <div
                    ...
                    style={{
                      borderColor:
                        "color-mix(in srgb, " +
                        (form.theme_text || THEME_DEFAULTS.theme_text) +
                        " 12%, transparent)",
                      background:
                        form.theme_surface || THEME_DEFAULTS.theme_surface,
                    }}
                  >
                    <div>
                      <p
                    ...
                    <div className="text-right">
                      <p
                        className="font-num text-2xl leading-none"
                        style={{
                          color:
                            form.theme_primary || THEME_DEFAULTS.theme_primary,
                        }}
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        ...
                        >
                          ...
                          <p className="font-num text-base leading-tight">
                            {p.time}
                          </p>
                        </div>
                      ))}
          ...
          <div className="max-w-5xl space-y-6">
            <div>
              <h2 className="font-display text-xl text-white">
                Pengaturan PWA (Aplikasi Terinstall)
              </h2>
              <p className="mt-1 text-sm text-emerald-100/55">
                Atur identitas aplikasi saat dipasang ke desktop, laptop,
                tablet, Android, dan iOS. Perubahan tersimpan ke database dan
                diterapkan real-time ke manifest &amp; tampilan aplikasi.
              </p>
            </div>

            <div className="grid gap-6 lg:grid-cols-2">
              <div className="flex flex-col gap-2">
                <label className={labelClass} htmlFor="pwa_app_name">
                  Nama Aplikasi
                </label>
                <input
                  id="pwa_app_name"
              ...
              <div className="flex flex-col gap-2">
                <label className={labelClass} htmlFor="pwa_short_name">
                  Nama Singkat
                </label>
                <input
                  id="pwa_short_name"
                  ...
                  onChange={(e) =>
                    setForm({ ...form, pwa_short_name: e.target.value })
                  }
                  placeholder="Masjid"
                />
                <p className="text-xs text-emerald-100/45">
                  Tampil di ikon home screen (maks. 30 karakter).
                </p>
              </div>
              <div className="flex flex-col gap-2 lg:col-span-2">
                <label className={labelClass} htmlFor="pwa_description">
                  Deskripsi Aplikasi
                </label>
                <textarea
                  id="pwa_description"
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="flex flex-col gap-3 border border-white/10 bg-white/[0.03] p-5">
                <label className={labelClass} htmlFor="pwa_theme_color">
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="h-11 w-14 shrink-0 cursor-pointer rounded border border-white/15 bg-transparent p-0"
                    value={
                      /^#[0-9a-fA-F]{6}$/.test(form.pwa_theme_color || "")
                        ? form.pwa_theme_color
                        : "#04100c"
                    }
                    onChange={(e) =>
                      setForm({ ...form, pwa_theme_color: e.target.value })
                    }
                  />
                  <input
                    className={inputClass}
                ...
                <p className="text-xs text-emerald-100/45">
                  Warna bilah status / judul jendela aplikasi terinstall.
                </p>
              </div>
              <div className="flex flex-col gap-3 border border-white/10 bg-white/[0.03] p-5">
                <label className={labelClass} htmlFor="pwa_bg_color">
                  Warna Latar (Splash)
                </label>
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="h-11 w-14 shrink-0 cursor-pointer rounded border border-white/15 bg-transparent p-0"
                    value={
                      /^#[0-9a-fA-F]{6}$/.test(form.pwa_bg_color || "")
                        ? form.pwa_bg_color
                        : "#04100c"
                    }
                    onChange={(e) =>
                      setForm({ ...form, pwa_bg_color: e.target.value })
                    }
                  />
                  <input
                    className={inputClass}
                ...
                <p className="text-xs text-emerald-100/45">
                  Warna latar layar splash saat aplikasi dibuka.
                </p>
              </div>
            </div>

            <div className="flex flex-col gap-3 border border-white/10 bg-white/[0.03] p-5">
              <label className={labelClass} htmlFor="pwa_logo">
                Logo / Ikon Aplikasi
              </label>
              <input
                id="pwa_logo"
              ...
              />
              <p className="text-xs text-emerald-100/45">
                Mendukung PNG, JPG, SVG, ICO. Maks. 5 MB. Digunakan sebagai ikon
                aplikasi pada semua ukuran (192/512/1024) &amp;
                apple-touch-icon.
              </p>
              <div className="mt-2 flex flex-wrap items-center gap-4">
                {pwaLogoFile ? (
                  <div className="flex items-center gap-3">
                    <img
                      src={URL.createObjectURL(pwaLogoFile)}
                      alt="Pratinjau logo PWA"
                      className="h-16 w-16 rounded object-contain bg-white/5 ring-1 ring-white/10"
                    />
                    <span className="text-xs text-emerald-100/55">
                      {pwaLogoFile.name} (belum disimpan)
                    </span>
                  </div>
                ) : settings && settings.pwa_logo ? (
                  <div className="flex items-center gap-3">
                    <img
                      src={fileUrl(settings, settings.pwa_logo)}
                      alt="Logo PWA saat ini"
                      className="h-16 w-16 rounded object-contain bg-white/5 ring-1 ring-[color-mix(in_srgb,var(--m-primary)_50%,transparent)]"
                    />
                    <span className="text-xs text-emerald-100/55">
                      Logo PWA saat ini
                    </span>
