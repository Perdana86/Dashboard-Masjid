...
const AdminLoginPage = () => {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            setError(err && err.status === 400 ? 'Email atau kata sandi salah.' : 'Gagal masuk. Coba lagi.');
        } finally {
            setBusy(false);
        }
    };
    ...
    return (
        <div className="masjid-pattern flex min-h-[100dvh] items-center justify-center bg-[var(--m-bg)] px-6 py-16 text-emerald-50">
            <Helmet>
                <title>Login Admin | Dashboard Masjid</title>
                <meta name="description" content="Halaman masuk pengurus masjid untuk mengelola jadwal, slide informasi, dan running teks dashboard." />
            </Helmet>
            ...
            <div className="w-full max-w-md border border-white/10 bg-[color-mix(in_srgb,var(--m-bg)_80%,transparent)] p-8 backdrop-blur">
                <div className="mb-7 flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="font-display text-2xl text-white">Masuk Panel Admin</h1>
                        <p className="text-sm text-emerald-100/60">Khusus pengurus masjid.</p>
                    </div>
                </div>

                <form onSubmit={onSubmit} className="space-y-5">
                    <div className="flex flex-col gap-2">
                        <label htmlFor="email" className="text-xs uppercase tracking-[0.2em] text-emerald-100/60">Email</label>
                        <input
                            id="email"
                            type="email"
                            required
                    ...
                    <div className="flex flex-col gap-2">
                        <label htmlFor="password" className="text-xs uppercase tracking-[0.2em] text-emerald-100/60">Kata Sandi</label>
