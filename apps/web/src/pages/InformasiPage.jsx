...
const InformasiPage = () => {
  ...
  const current = items.length
    ? items[Math.min(slideIndex, items.length - 1)]
    : null;
  ...
  useEffect(() => {
    ...
    async function updateContrast() {
      ...
      if (current.slide_type === "text") {
        const luma = parseHexLuma(
          current.text_bg || current.text_background_color,
        );
  ...
  const textMuted = headerOnLight
    ? "text-emerald-900/70"
    : "text-emerald-100/70";
  const labelColor = headerOnLight
    ? "text-emerald-800"
    : "text-[var(--m-primary)]";
  ...
  return (
    <div className="relative h-[100dvh] w-full overflow-hidden bg-[var(--m-bg)] text-emerald-50">
      ...
      <header className="pointer-events-none absolute inset-x-0 top-0 z-30">
        <div
          className={`pointer-events-auto transition-colors duration-500 ${scrim}`}
        >
