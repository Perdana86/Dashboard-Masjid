...
const BioPage = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!isAbortError(e) && mounted)
          setError(e.message || "Gagal memuat bio masjid");
      } finally {
    ...
    const unsubBio = subscribeCollection("bio", () => {
      if (!mounted) return;
      fetchBio({ requestKey: "bio-page-bio-rt" })
        .then((b) => {
          if (mounted) setBio(b);
        })
        .catch(() => {});
    ...
    const unsubSettings = subscribeCollection("settings", () => {
      if (!mounted) return;
      fetchSettings({ requestKey: "bio-page-settings-rt" })
        .then((s) => {
    ...
    const unsubLinks = subscribeCollection("bio_links", () => {
      ...
      fetchBioLinks({ requestKey: "bio-page-links-rt" })
        .then((list) => {
          if (mounted)
            setLinks((list || []).filter((item) => item.active !== false));
        })
        .catch(() => {});
    });
  ...
  const mosqueName =
    (bio && bio.mosque_name) ||
    (settings && settings.mosque_name) ||
    "Dashboard Masjid";

  const contactItems = [
    bio && bio.address
      ? { icon: MapPin, label: "Alamat", value: bio.address }
      : null,
    bio && bio.phone
      ? { icon: Phone, label: "Telepon", value: bio.phone }
      : null,
    bio && bio.email ? { icon: Mail, label: "Email", value: bio.email } : null,
    bio && bio.operating_hours
      ? { icon: Clock, label: "Jam Operasional", value: bio.operating_hours }
      : null,
  ].filter(Boolean);
    ...
    <div className="min-h-[100dvh] bg-[var(--m-bg)] text-emerald-50">
      ...
      <div className="masjid-pattern flex min-h-[100dvh] flex-col">
        ...
        <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-8 md:px-10 md:py-12">
          {loading ? (
            ...
            <div className="border border-white/10 bg-white/[0.03] p-8 text-center text-emerald-100/60">
              Konten bio belum diisi. Silakan atur di Panel Admin &rarr; tab
              Bio.
            </div>
          ) : (
                ...
                {photo ? (
                  <div className="relative aspect-[16/9] w-full overflow-hidden md:aspect-[21/9]">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-5 md:p-8">
                      ...
                      {bio.description ? (
                        <p className="mt-2 max-w-2xl text-sm text-emerald-100/80 md:text-base">
                          {bio.description}
                        </p>
                      ) : null}
                    </div>
                  ...
                  {contactItems.map((item, i) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 break-words text-sm text-emerald-50/90 md:text-base">
                          {item.value}
                        </p>
                      </div>
                    </motion.div>
                        ...
                        >
                          <span className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded border border-[color-mix(in_srgb,var(--m-primary)_40%,transparent)] bg-[var(--m-surface)]/60">
                            {icon ? (
                              ...
                              <LinkIcon
                                className="h-5 w-5 text-[var(--m-primary)]"
                                strokeWidth={1.75}
                              />
                            )}
                          </span>
                          <span className="min-w-0 flex-1">
                            <span className="block truncate text-sm font-medium text-emerald-50/90 md:text-base">
                              {link.label}
                            </span>
                          </span>
                          <ExternalLink
                            className="h-4 w-4 shrink-0 text-emerald-100/40 transition-colors group-hover:text-[var(--m-primary)]"
                            strokeWidth={1.75}
                          />
                        </motion.a>
                      );
        ...
        <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-white/10 px-6 py-3 text-xs text-emerald-100/45 md:px-10">
          <span>
            &copy; {new Date().getFullYear()} {mosqueName}
          </span>
          <div className="flex flex-wrap items-center gap-3">
            <Link
              to="/"
              className="inline-flex min-h-11 items-center gap-1 hover:text-[var(--m-primary)]"
            >
