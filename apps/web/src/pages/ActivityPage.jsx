...
const ActivityPage = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!isAbortError(e) && mounted)
          setError(e.message || "Gagal memuat kegiatan masjid");
      } finally {
    ...
    const unsubList = subscribeCollection("activities", () => {
      ...
      fetchActivities({ requestKey: "activity-page-list-rt" })
        .then((list) => {
          if (mounted)
            setActivities((list || []).filter((item) => item.active !== false));
        })
        .catch(() => {});
    });
    const unsubSettings = subscribeCollection("settings", () => {
      if (!mounted) return;
      fetchSettings({ requestKey: "activity-page-settings-rt" })
        .then((s) => {
  ...
  const mosqueName = (settings && settings.mosque_name) || "Dashboard Masjid";

  const renderMedia = (item) => {
    const imageUrl = item.image ? fileUrl(item, item.image) : "";
    const localVideoUrl = item.video_local
      ? fileUrl(item, item.video_local)
      : "";
    ...
    return (
      <div
        className="relative w-full overflow-hidden bg-[var(--m-surface)]"
        style={{ aspectRatio: "16 / 9" }}
      >
        {imageUrl ? (
          <img
          ...
          <div className="flex h-full w-full items-center justify-center">
            <CalendarDays
              className="h-12 w-12 text-[var(--m-primary)]/40"
              strokeWidth={1.5}
            />
          </div>
        )}
        ...
        <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-8 md:px-10 md:py-12">
          {loading ? (
            ...
            <div className="border border-white/10 bg-white/[0.03] p-8 text-center text-emerald-100/60">
              Belum ada kegiatan yang ditampilkan. Silakan tambahkan kegiatan di
              Panel Admin &rarr; tab Kegiatan.
            </div>
          ) : (
