...
import { fileUrl, fetchSettings, isAbortError, subscribeCollection } from '@/lib/masjid';
...
const STATIC_ICONS = [
    ...
    { src: '/icons/icon-512.png', sizes: '1024x1024', type: 'image/png', purpose: 'any' },
    { src: '/icons/maskable-512.png', sizes: '192x192', type: 'image/png', purpose: 'maskable' },
    { src: '/icons/maskable-512.png', sizes: '512x512', type: 'image/png', purpose: 'maskable' },
    { src: '/icons/maskable-512.png', sizes: '1024x1024', type: 'image/png', purpose: 'maskable' },
];
...
export default function usePwaManifest() {
    ...
    useEffect(() => {
        const apply = (settings) => {
            const appName = (settings && settings.pwa_app_name) || DEFAULTS.pwa_app_name;
            const shortName = (settings && settings.pwa_short_name) || DEFAULTS.pwa_short_name;
            const description = (settings && settings.pwa_description) || DEFAULTS.pwa_description;
            const themeColor = (settings && settings.pwa_theme_color) || DEFAULTS.pwa_theme_color;
            const bgColor = (settings && settings.pwa_bg_color) || DEFAULTS.pwa_bg_color;
        ...
        const unsubSettings = subscribeCollection('settings', () => {
            ...
            fetchSettings({ requestKey: 'pwa-manifest-settings-rt' })
                .then((s) => { if (mounted) { latest = s; apply(latest); } })
                .catch((e) => { if (!isAbortError(e)) { /* leave static manifest */ } });
        });
