...
export function nextPrayer(jadwal, minutesOfDay) {
    ...
    const upcoming = schedule.find((p) => p.minutes > minutesOfDay) || schedule[0];
...
export function scheduleConfig(settings) {
    return {
        before: Number((settings && settings.sched_before_prayer) || SCHEDULE_DEFAULTS.sched_before_prayer),
        after: Number((settings && settings.sched_after_prayer) || SCHEDULE_DEFAULTS.sched_after_prayer),
        interval: Number((settings && settings.sched_interval_hours) || SCHEDULE_DEFAULTS.sched_interval_hours),
...
export function activeSlideSource(jadwal, minutesOfDay, settings) {
    ...
    if (!cfg.enabled) return { source: 'dashboard', reason: 'Penjadwalan nonaktif' };
    ...
    if (jadwal) {
        ...
        for (const p of times) {
            if (minutesOfDay >= p.minutes - cfg.before && minutesOfDay < p.minutes) {
                return { source: 'dashboard', reason: `${cfg.before} menit sebelum ${p.key}` };
            }
            if (minutesOfDay >= p.minutes && minutesOfDay <= p.minutes + cfg.after) {
                return { source: 'info', reason: `${cfg.after} menit setelah ${p.key}` };
            }
        }
    }
...
export async function fetchJadwal(cityId, isoDate) {
    const res = await apiServerClient.fetch(`/sholat/jadwal?city=${encodeURIComponent(cityId)}&date=${isoDate}`);
...
export async function fetchSlides(opts = {}) {
    ...
    return pb.collection('slides').getFullList({ sort: 'position,created', requestKey: opts.requestKey ?? null });
}

export async function fetchDashboardSlides(opts = {}) {
    if (shouldUseSupabase() && !opts.forcePocketBase) return fetchDashboardSlidesSupa();
    return pb.collection('dashboard_slides').getFullList({ sort: 'position,created', requestKey: opts.requestKey ?? null });
}
...
export async function fetchActivities(opts = {}) {
    if (shouldUseSupabase() && !opts.forcePocketBase) return fetchActivitiesSupa();
...
export function fileUrl(record, filename) {
    ...
    if (filename.startsWith('/hcgi/platform/') || filename.startsWith('/api/files/')) return filename;
...
export function subscribeCollection(collection, onChange) {
    ...
    return () => {
        ...
        void p.catch(() => {});
        void pb.collection(collection).unsubscribe('*').catch(() => {});
    };
}
...
export function slideVideo(slide) {
    if (slide.video && typeof slide.video === 'string') return fileUrl(slide, slide.video);
...
export function themeColors(settings) {
    return {
        ...
        surface: (settings && settings.theme_surface) || THEME_DEFAULTS.theme_surface,
        primary: (settings && settings.theme_primary) || THEME_DEFAULTS.theme_primary,
...
export function slideMediaType(slide) {
    ...
    const file = slide.image && typeof slide.image === 'string' ? slide.image : '';
...
export function isVideoSlide(slide) {
    ...
    const file = slide.video && typeof slide.video === 'string' ? slide.video : '';
